...
(hash curl 2>/dev/null || sudo apt -y install curl) &&
    ...
    curl -z nim.tar.xz -o nim.tar.xz -L $(curl -sSL https://api.github.com/repos/nim-lang/nightlies/releases | jq -r "[ .[]?.assets[] | select(.browser_download_url | test(\"latest-devel/linux_$lArchitecture\")) | {updated_at, browser_download_url} ] | sort_by(.updated_at) | reverse | .[0].browser_download_url") &&
    rm -rf "$HOME/.nim" &&
    tar -xvf nim.tar.xz &&
    rm nim.tar.xz || true &&
    mv nim-* "$HOME/.nim" &&
